...
速度転送に加え、ヘッドライト・タワーライト・バンパー・ブレーキなどの IO デバイスへの制御指令を受け付け、コントローラステータス・エンコーダカウント・モータドライバ温度・エラーコード・バンパー/ブレーキ設定値などのセンサ情報を ROS 側へ転送します。
...
### ハードウェア
- Raspberry Pi Pico 2 W(CuGo V4.5 に同梱)
...
- [RPi_Pico_TimerInterrupt](https://github.com/khoih-prog/RPI_PICO_TimerInterrupt) — CRST01A の通信タイミング管理
...
3. Arduino IDE のライブラリマネージャから `PacketSerial`, `RPi_Pico_TimerInterrupt` をインストールします。
...
6.  Arduino IDE を開いたPCとCuGo V4.5 に搭載されたRaspberry Pi Pico 2 W をUSBケーブルで接続し、プログラムを書き込みます。


以上でRaspberry Pi Pico 2 W側の手順は完了となります。引き続き、[cugo_v4.5_ros2_controlのreadme](https://github.com/CuboRex-Development/cugo_v4.5_ros2_control)のInstallation、 Usage の手順を実施してください。
...
基板上のボックスコネクタに配線を接続し、Raspberry Pi Pico 2 W のUARTピンを使用するモードです。UART1(GP8: TX / GP9: RX)を使用してROS 2 PC と通信します。USB-Serial 接続と同じプロトコルで動作します。
...
通信速度は `BOX_CN_BAUD_RATE` で変更できます(デフォルト: 115200)。ROS 2 PC 側の設定と一致させてください。

```cpp
#define BOX_CN_BAUD_RATE    (115200)    // ← 必要に応じて変更 (例: 921600)
```
...
Raspberry Pi Pico 2 W の Classic Bluetooth(SPP：Serial Port Profile)を使用してROS 2 PCと通信します。PC側では仮想シリアルポートとして認識されるため、ペアリング後はほかのシリアルモードと同様に扱えます。
...
スケッチを書き込むと、Raspberry Pi Pico 2 W が Bluetooth デバイスとして起動します。`INFO_SERIAL` が有効な場合、USB シリアルモニタに以下のように表示されます。
...
Raspberry Pi Pico 2 W 自身をアクセスポイントとして動作させます。WiFiルータは不要です。
...
スケッチを書き込むと、Raspberry Pi Pico 2 W がアクセスポイントとして起動します。ROS PC をそのアクセスポイントに接続し、Pico の IP アドレス(デフォルト: `192.168.42.1`)を ROS 側の接続先として指定してください。
...
Raspberry Pi Pico 2 W と ROS 2 PC を、外部のWiFiルータに接続して動作させます。WiFiルータはご自身でご用意ください。
...
スケッチを書き込むと、Raspberry Pi Pico 2 W は設定した SSID の WiFi ルータに接続します。
...
ハンドシェイク完了後、受信パケットの body[6] (enable_7_14) に設定されたビットに対応する IO 制御指令を処理します。

| enable_7_14 ビット | 対象フィールド | 処理内容                                       |
| :----------------: | :------------: | ---------------------------------------------- |
|       bit0        |   body[7]      | 走行モード切替 (0x80=RC / 0x81=CMD)            |
|       bit1        |   body[8]      | 緊急減速発動                                   |
|       bit2        |   body[9]      | コントローラエラー解除                         |
|       bit3        |   body[10]     | モータドライバエラー解除                       |
|       bit4        |   body[11]     | ヘッドライト制御                               |
|       bit5        |   body[12]     | タワーライト制御                               |
|       bit6        |   body[13]     | バンパー設定書き込み                           |
|       bit7        |   body[14]     | ブレーキ設定書き込み                           |

応答パケットには、以下のセンサデータを body[6-59] に格納して送信します。

| body オフセット | 内容                               | 型        | 取得元              |
| :-------------: | ---------------------------------- | --------- | ------------------- |
|      [6]        | コントローラステータス             | uint8_t   | 0x80 (周期)         |
|      [7]        | コントローラエラービット           | uint8_t   | 0x80 (周期)         |
|      [8]        | モータドライバエラービット         | uint8_t   | 0x80 (周期)         |
|    [9-10]       | ドライバ電圧 raw (× 0.1 V)         | uint16_t  | 0x80 (周期)         |
|      [11]       | ヘッドライト状態                   | uint8_t   | 0x84 (周期)         |
|      [12]       | タワーライト状態                   | uint8_t   | 0x84 (周期)         |
|      [13]       | 外部 4bit デジタル入力             | uint8_t   | 0x84 (周期)         |
|   [14-29]       | エンコーダカウント (モータ 0〜3)   | uint32_t×4| 0x88/0x89 (周期)    |
|   [30-37]       | モータドライバ温度 [℃] (0〜3)     | uint16_t×4| 0x8C (周期)         |
|   [38-45]       | モータドライバエラーコード (0〜3)  | uint16_t×4| 0x8E (周期)         |
|      [46]       | バンパー設定値 (フラッシュ読み出し)| uint8_t   | 0xC4 (1秒ごと要求)  |
|      [47]       | ブレーキ設定値 (フラッシュ読み出し)| uint8_t   | 0xC4 (1秒ごと要求)  |
|      [59]       | データ読み取りエラーフラグ (bit0=1でエラー)| uint8_t   | —                   |

バンパー・ブレーキ設定値は `Job1000ms()` で 1 秒ごとに非ブロッキングで要求し、最新のキャッシュを返答に含めます。

body[59] の bit0 は、CRST01A から取得する各種データのいずれかが未受信またはタイムアウトになった場合に `1` となり、全データが正常に受信できている場合は `0` となります。このフラグが `1` の場合、速度指令は CRST01A に転送されません。
