...
追加設定は不要です。`config.h` の `USE_BOX_CN`・`USE_WIFI` 行がコメントアウトされていることを確認してください。
...
基板上のボックスコネクタにあるボックスコネクタに配線を接続し、Raspberry Pi Pico 2WHのUARTピンを使用するモードです。UART1（GP8: TX / GP9: RX）を使用してROS 2 PC と通信します。USB-Serial 接続と同じプロトコルで動作します。

使用するためには、`config.h` の `USE_BOX_CN` のコメントを外します。

```cpp
#define USE_BOX_CN   // ← コメントを外す (BOX_CN-Serial 接続)
...
WiFi モードでは `config.h` に以下のデバッグフラグを用意しています。コメントを外すと USB シリアルモニタに対応するログが出力されます。
...
> [!WARNING]
> `DEBUG_WIFI_*` フラグは USB-Serial モードおよび BOX_CN-Serial モード（`USE_WIFI` 未定義時）では使用できません。これらのモードでは Serial ポートを ROS との通信に使用するため、ログ出力と競合します。
